...
Public Class FormSuppliers
    ...
    '-------------------------------------------ЗАПОЛНЕНИЕ ГРИДА ПОСТАВЩИКОВ------------------------
    Private Sub Grid()
        Dim MyConnection As SqlConnection
        Dim MyDataAdapter As SqlDataAdapter
        MyConnection = New SqlConnection(ClassAuth.ClassAuth.con)
        MyDataAdapter = New SqlDataAdapter("Adm_Suppliers", MyConnection)
        ...
        col1.Name = "SUPPLIER_ID"
        col1.ReadOnly = True
        col1.HeaderText = "ID"
        col1.DataPropertyName = "SUPPLIER_ID"
        ...
        col2.Name = "SUPPLIER_NAME"
        col2.ReadOnly = False
        col2.HeaderText = "Название"
        col2.DataPropertyName = "SUPPLIER_NAME"
        ...
        colAdres.HeaderText = "Адрес поставщика"
        ...
        DataGridView1.Sort(DataGridView1.Columns("SUPPLIER_NAME"), System.ComponentModel.ListSortDirection.Ascending)
    End Sub
    ...
    Private Sub DataGridView1_CellValueChanged(sender As Object, e As DataGridViewCellEventArgs) Handles DataGridView1.CellValueChanged
        ...
        If errVvod = 0 Then
            Try
                ...
                MyDataAdapter.Parameters("@SUPPLIER_ID").Value = DataGridView1.CurrentRow.Cells(DataGridView1.Columns("SUPPLIER_ID").Index).Value
                '---------------------------------------------------------------------------------------
                MyDataAdapter.Parameters.Add(New SqlParameter("@SUPPLIER_NAME", SqlDbType.VarChar, 100))
                MyDataAdapter.Parameters("@SUPPLIER_NAME").Value = DataGridView1.CurrentRow.Cells(DataGridView1.Columns("SUPPLIER_NAME").Index).Value
                '---------------------------------------------------------------------------------------
    ...
    Private Sub DataGridView1_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles DataGridView1.CellContentClick
        If e.ColumnIndex = DataGridView1.Columns("DelRow").Index Then
            Try
                Y = DataGridView1.CurrentCellAddress.Y
                Dim DRes As DialogResult = MsgBox("Вы уверены, что хотите удалить поставщика """ & DataGridView1.Rows.Item(Y).Cells.Item(DataGridView1.Columns("SUPPLIER_NAME").Index).Value & """?", vbYesNo)
    ...
    '-------------------------------------------АКТИВАЦИЯ ФОРМЫ---------------------------------------------
    Private Sub FormSuppliers_Activated(sender As Object, e As EventArgs) Handles MyBase.Activated
        If FormSuppliersAdd.FSAClosed IsNot Nothing Then
            If FormSuppliersAdd.FSAClosed = True Then
                Grid()
                FormSuppliersAdd.FSAClosed = False
            End If
        End If
    End Sub
